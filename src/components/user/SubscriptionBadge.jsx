...
const PLAN_META = {
  ...
  PREMIUM: { color: "#C9963A", bg: "rgba(201,150,58,0.15)", label: "Premium", perks: ["Pro features", "Public page customization", "Choose a custom public URL", "Premium templates"] },
};
...
export default function SubscriptionBadge() {
  ...
  useEffect(() => {
    loadPlan();
    const handleRefresh = () => loadPlan();
    window.addEventListener("subscription:changed", handleRefresh);
    window.addEventListener("focus", handleRefresh);
    return () => {
      window.removeEventListener("subscription:changed", handleRefresh);

...
const ResumeEditorPage = () => {
  ...
  useEffect(() => {
    ...
    const load = () => {
      switch (tab) {
        case "Experience":     return axios.get(`/api/experiences/resume/${resumeId}`).then((r) => setExperiences(r.data));
        case "Education":      return axios.get(`/api/educations/resume/${resumeId}`).then((r) => setEducations(r.data));
        case "Skills":         return axios.get(`/api/skills/resume/${resumeId}`).then((r) => setSkills(r.data));
        case "Projects":       return axios.get(`/api/projects/resume/${resumeId}`).then((r) => setProjects(r.data));
        case "Certifications": return axios.get(`/api/certifications/resume/${resumeId}`).then((r) => setCertifications(r.data));
        case "Publications":   return axios.get(`/api/publications/resume/${resumeId}`).then((r) => setPublications(r.data));
        case "Testimonials":   return axios.get(`/api/testimonials/resume/${resumeId}`).then((r) => setTestimonials(r.data));
        case "Services":       return axios.get(`/api/services/resume/${resumeId}`).then((r) => setServices(r.data));
        case "Exhibitions":    return axios.get(`/api/exhibitions/resume/${resumeId}`).then((r) => setExhibitions(r.data));
        case "Financial":      return axios.get(`/api/financials/resume/${resumeId}`).then((r) => setFinancials(r.data));
        case "Blogs":          return axios.get(`/api/blogs/resume/${resumeId}`).then((r) => setBlogs(r.data));
        case "Contacts":       return axios.get(`/api/contacts/resume/${resumeId}`).then((r) => setContacts(r.data));
        case "Versions":       return axios.get(`/api/resumes/${resumeId}/versions`).then((r) => setVersions(r.data));
  ...
  const saveMeta = () => {
    axios
      .patch(`/api/resumes/${resumeId}/meta`, {
        title: metaForm.title,
        professionType: metaForm.profession
      })
  ...
  const changeTheme = (themeId) => {
    axios
      .patch(`/api/resumes/${resumeId}/theme`, { themeId })
  ...
  const submitForApproval = () => {
    axios
      .post(`/api/resumes/${resumeId}/submit`)
  ...
  /* ── Publish resume ── */
  const publishResume = () => {
    if (!window.confirm("Publish this resume? It will be visible to the public.")) return;
    axios
      .post(`/api/resumes/${resumeId}/publish`)
      .then((r) => { setResume(r.data); setMsg("Resume published!"); })
      .catch(() => setMsg("Failed to publish."));
  };

  /* ── Unpublish resume ── */
  const unpublishResume = () => {
    if (!window.confirm("Unpublish this resume? It will no longer be visible to the public.")) return;
    axios
      .post(`/api/resumes/${resumeId}/unpublish`)
      .then((r) => { setResume(r.data); setMsg("Resume unpublished!"); })
      .catch(() => setMsg("Failed to unpublish."));
  };
  ...
  const createSnapshot = () => {
    ...
    axios
      .post(`/api/resumes/${resumeId}/versions`, { changeNote: note })
      .then(() => { setMsg("Snapshot created!"); axios.get(`/api/resumes/${resumeId}/versions`).then((r) => setVersions(r.data)); })
      ...
      rightAction={
        <div style={{ display: "flex", gap: "0.5rem" }}>
          ...
          {resume?.approvalStatus === "APPROVED" && !resume?.published && (
            <button onClick={publishResume} style={{ background: "#28a745", color: "white" }}>🚀 Publish</button>
          )}
          {resume?.published && (
            <button onClick={unpublishResume} style={{ background: "#dc3545", color: "white" }}>⏸️ Unpublish</button>
          )}
    ...
    >
      ...
      {tab === "Meta" && (
        <div>
          ...
          <h3>Theme</h3>
          <p>Current theme: <strong>{themes.find(t => t.id === resume?.themeId)?.name || "Default"}</strong></p>
          <select
